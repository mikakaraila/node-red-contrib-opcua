...
import {
  UaDiscoveryNode,
  UaDiscoveryDef
} from "./109-opcuadiscoverydef";

import { OPCUACertificateManager, makeApplicationUrn, OPCUADiscoveryServer, findServers
 } from "node-opcua";
...
const UaDiscovery: NodeInitializer = (RED): void => {
  function UaDiscoveryNodeConstructor(
      this: UaDiscoveryNode,
      n: UaDiscoveryDef
  ): void {
    ...
    this.name = n.name;
    /* eslint-disable-next-line */
    const node: UaDiscoveryNode = this;
    ...
    const server_options = {
      ...
      serverInfo: {
        applicationUri: makeApplicationUrn(os.hostname(), "Node-Red-OPCUA-Discovery"),
    ...
    const server = new OPCUADiscoveryServer(server_options);
    ...
    (async () => {
      try {
        ...
        /* eslint-disable-next-line */
        await server.start(function (err: any) {
          if (err) {
            node.status({
              fill: "red",
              shape: "ring",
